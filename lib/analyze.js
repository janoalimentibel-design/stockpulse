...
export function computeAnalysis(data) {
  const { price, ma50, ma200, high52, low52, change1m, rsi, macd, macdSignal, relVol, pe, peSector, epsGrowth, netMargin, de, divYield, roe } = data
  ...
  const total = bull + bear + (indicators.length - bull - bear)
  const score = total > 0 ? Math.round(bull / indicators.length * 100) : 0
  const neutralCount = indicators.length - bull - bear
  ...
  const confidence = indicators.length >= 8 ? 'Alta' : indicators.length >= 5 ? 'Media' : 'Baja'

  return { indicators, bull, bear, neutral: neutralCount, total: indicators.length, score, trend, signal, sentiment, confidence }
}
