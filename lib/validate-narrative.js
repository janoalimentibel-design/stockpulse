...
// R7: sanity check con Haiku — verifica contradicciones entre narrativa y datos numéricos
export async function r7HaikuSanityCheck(narrative, dataset) {
  const key = process.env.ANTHROPIC_API_KEY
  if (!key) {
    console.warn('[validate-narrative] R7: ANTHROPIC_API_KEY ausente — saltando sanity check')
    return { hasIssues: false, issues: [] }
  }

  try {
    // Construir datasetContext con solo campos no-null
    const contextParts = []
    if (dataset.ticker)      contextParts.push(`Ticker: ${dataset.ticker}`)
    if (dataset.price != null)       contextParts.push(`Precio: $${dataset.price}`)
    if (dataset.rsi != null)         contextParts.push(`RSI: ${dataset.rsi}`)
    if (dataset.ma50 != null)        contextParts.push(`MA50: $${dataset.ma50}`)
    if (dataset.ma200 != null)       contextParts.push(`MA200: $${dataset.ma200}`)
    if (dataset.change1m != null)    contextParts.push(`Cambio 1 mes: ${dataset.change1m}%`)
    if (dataset.panelTrend)          contextParts.push(`Tendencia panel: ${dataset.panelTrend}`)
    if (dataset.macd != null)        contextParts.push(`MACD: ${dataset.macd}`)
    if (dataset.macdSignal != null)  contextParts.push(`Señal MACD: ${dataset.macdSignal}`)
    const datasetContext = contextParts.join(' | ')

    const narrativeText = [
      narrative.technical_summary   ? `TÉCNICO: ${narrative.technical_summary}`   : null,
      narrative.fundamental_summary ? `FUNDAMENTAL: ${narrative.fundamental_summary}` : null,
      narrative.analyst_summary     ? `ANALISTA: ${narrative.analyst_summary}`     : null,
      narrative.key_opportunity     ? `OPORTUNIDAD: ${narrative.key_opportunity}`  : null,
      narrative.key_risk            ? `RIESGO: ${narrative.key_risk}`              : null,
    ].filter(Boolean).join('\n')

    const haikuPrompt = `Sos un revisor de calidad financiera. El siguiente análisis fue generado automáticamente.
...
Si no encontrás problemas: {"hasIssues": false, "issues": []}`
    ...
    if (!res.ok) {
      console.warn(`[validate-narrative] R7: Haiku HTTP ${res.status} — saltando`)
      return { hasIssues: false, issues: [] }
    }
    ...
    if (s === -1 || e === -1) return { hasIssues: false, issues: [] }
    const result = JSON.parse(raw.slice(s, e + 1))
    return {
      hasIssues: Boolean(result.hasIssues),
      issues: Array.isArray(result.issues)
        ? result.issues.map(i => `R7: ${String(i)}`).slice(0, 5)
        : [],
    }
  } catch (err) {
    console.warn('[validate-narrative] R7: error en Haiku sanity check —', err.message)
    return { hasIssues: false, issues: [] }
  }
}
